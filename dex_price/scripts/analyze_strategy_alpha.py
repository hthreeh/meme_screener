...
# -*- coding: utf-8 -*-
"""
分析策略 Alpha (Alpha Score System) 的交易表现
"""
import sqlite3
import pandas as pd
import os
...
def analyze_strategy_alpha():
    print("=== 开始分析策略 Alpha (Strategy Alpha) [评分系统] ===")
    
    conn = get_connection()
    
    try:
        # 获取策略状态
        state_df = pd.read_sql_query("SELECT * FROM strategy_states WHERE strategy_type='Alpha'", conn)
        ...
        df = pd.read_sql_query("SELECT * FROM strategy_trades WHERE strategy_type='Alpha' ORDER BY timestamp", conn)
